  ...
  it("splits assistant content into typed blocks in transcript order", () => {
    const message = {
      ...
      content: [
        ...
        { type: "toolCall", id: "call_1", name: "grep", arguments: { pattern: "foo" } },
      ],
    };

    const blocks = classifyBlocks(message, 0);

    expect(blocks.map((block) => block.blockType)).toEqual(["thinking", "text", "tool-call"]);
    expect(blocks.map((block) => block.label)).toEqual(["thinking", "text", "tool-call · grep"]);
