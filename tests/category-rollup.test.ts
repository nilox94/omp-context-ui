...
import {
  ...
  formatCategoryRow,
  formatContextFooter,
} from "../src/overlay/category-rollup.js";
...
describe("formatCategoryRow", () => {
  it("renders label and approximate token total", () => {
    expect(formatCategoryRow({ label: "Messages", tokens: 142_000 })).toBe("Messages  ~142k");
  });
});
...
describe("formatContextFooter", () => {
  it("shows percent and approximate totals", () => {
    expect(formatContextFooter(50_000, 200_000)).toBe("Context: 25% · ~50k / 200000");
  });
...
describe("buildCategoryRowsFromBreakdown", () => {
  it("maps all five breakdown categories in order", () => {
    const rows = buildCategoryRowsFromBreakdown({
      categories: [
        { id: "systemPrompt", label: "System prompt", tokens: 1000, color: "accent", glyph: "x" },
        { id: "systemTools", label: "System tools", tokens: 2000, color: "warning", glyph: "x" },
        { id: "systemContext", label: "System context", tokens: 3000, color: "customMessageLabel", glyph: "x" },
        { id: "skills", label: "Skills", tokens: 4000, color: "success", glyph: "x" },
        { id: "messages", label: "Messages", tokens: 5000, color: "userMessageText", glyph: "x" },
      ],
    });
    ...
    expect(rows.map((row) => row.tokens)).toEqual([1000, 2000, 3000, 4000, 5000]);
  });
});
