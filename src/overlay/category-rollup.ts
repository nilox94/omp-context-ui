...
export function computeContextUsagePercent(usedTokens: number, contextWindow: number): number | null {
  if (contextWindow <= 0) return null;
  return Math.round((usedTokens / contextWindow) * 100);
}

export function formatContextFooter(usedTokens: number, contextWindow: number): string {
