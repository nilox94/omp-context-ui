...
function buildMessageNodes(
	messages: readonly ContextSessionMessage[],
): ContextMessageNode[] {
...
function buildSystemPromptLeaves(
...
export function buildContextTree(session: ContextSessionSource): ContextTree {
	...
	const categoryChildren: Record<
		Exclude<CategoryId, "messages">,
		ContextLeafNode[]
	> = {
		systemPrompt: buildSystemPromptLeaves(systemPromptParts, skills),
		systemContext: buildLeafNodes(
			"systemContext",
			itemizeContextLeaves(systemPromptParts.slice(1)),
		),
		systemTools: buildToolLeaves(tools),
		skills: buildSkillLeaves(skills),
	};

	const categories: ContextCategoryNode[] = breakdown.categories.map(
		(category) => {
